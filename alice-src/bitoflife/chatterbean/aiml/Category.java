...
public class Category implements AIMLElement
{
  ...
  */
  ...
  public String process(Match match)
  {
	  //add
	  //获得属性值
	  TemplateElement[] childrens=template.getChildren();
      for(TemplateElement one:childrens)
      {  
    	  if(one instanceof Think)
    	  {
    		  Think current=(Think)one;
    		  List<TemplateElement> paras=current.children();
    		  if(paras!=null&&paras.size()>0&&(paras.get(0) instanceof Set))
    		  {
    			  for(TemplateElement para:paras)
        		  {
        			  Set attr=(Set)para;
        			  sets.put(attr.getName(), attr.process(match));
        		  }
    		  }
      ...
      {
    	  System.out.println("Category2: "+entry.getKey()+" "+entry.getValue());
      }
      */
